...
## Layout, as far as it is fixed

![The carrier drawn face by face, forward up: XIAO-ESP32S3-lora on the top face at y 35.5..56.5, XIAO-ESP32S3-cam on the bottom face at y 7.5..28.5, both USB-C ends aft, the underside BAT pads, the four mounting holes, and the free length left on each face](PCB-carrier-layout.svg)

Generated by [`gen_carrier_diagram.py`](../scripts/gen_carrier_diagram.py) from `PCB-carrier.kicad_pcb`. Regenerate it, do not edit it. It draws only what is placed: the outline, the holes and both XIAOs. The sensors, buzzer, L76K and JST are still numbers until [#14](https://github.com/jwilleke/js-rocket-avionics/issues/14) stage 2c. The battery is not on the carrier at all: it rides the PayloadSled, forward of it ([LiPo-500mAh.md](../LiPo-500mAh/LiPo-500mAh.md#where-it-goes--on-the-payloadsled-forward-of-the-carrier)).

- __A BAT pigtail leaves through an end of the XIAO, never a side.__ The header strips are 2.5 mm of plastic down both long sides. The pads sit 4.5 mm off centre on the D0..D6 side, __9.2 mm from the USB-C end and 11.8 mm from the far end__.
- __The two modules face each other across 7.0 mm__, on opposite faces: XIAO-ESP32S3-cam's far end at y 28.5, XIAO-ESP32S3-lora's USB-C end at y 35.5.
- __The bottom face has 3.5 mm to spare__ forward of XIAO-ESP32S3-cam: 66.5 mm clear against 63 needed.
- __Each face is drawn as seen by someone looking at it__, so the bottom panel is mirrored left to right against the top one. Drawn that way the two modules look identical, D0..D6 down the right. A module on the bottom face presents its __back__ to the shared holes, so on the board its rows are swapped against the top-face one: pins 1..7 on the x = 20.5 row, not 3.5. `gen_carrier.py` once netted it as if top-mounted ([#21](https://github.com/jwilleke/js-rocket-avionics/issues/21)), and the diagram script now refuses a board that does.
...
| 2b — nets and footprints | __partial__ — XIAOs placed and netted, [#18](https://github.com/jwilleke/js-rocket-avionics/issues/18) fixed and closed 2026-09-08 (`c2468eb`: the +8.5 row unmirrored, and `verify_pins()` now asserts every net against the footprint's own pin table on each run). [#21](https://github.com/jwilleke/js-rocket-avionics/issues/21) fixed 2026-09-10: XIAO-ESP32S3-cam is on the bottom face and had been netted as if top-mounted, so its rows are now swapped, and `verify_pins()` also checks every net against the underside photo, per face. Sensors, buzzer and JST still deferred to [#14](https://github.com/jwilleke/js-rocket-avionics/issues/14) |
