...
| __LDO__ | __Low-DropOut regulator.__ The chip that turns a higher, moving voltage into a steady lower one — on a XIAO, the battery's 4.2 V charged down to ~3.0 V flat becomes a steady __3.3 V__ on the `3V3` pin. It is __linear__, so it burns the difference as heat rather than converting it: (V in − V out) × current, which is ~__0.27 W__ at 300 mA in a sealed cone. Two facts follow that this project keeps running into — __two LDOs on one wire do not share the load__ (whichever targets the marginally higher voltage supplies everything until it sags to meet the other), and __an LDO's output is not an input__, which is why power goes to a sensor's `VIN` and never its `3Vo` |
