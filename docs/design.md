...
- __Shared battery couples the boards__ — and on the bench it did not matter: 67 minutes at full camera load, down to ~3.5 V, reset neither board ([#8](https://github.com/jwilleke/js-rocket-avionics/issues/8), [LiPo-500mAh.md](../hardware/LiPo-500mAh/LiPo-500mAh.md#the-known-risk-observed-no-brownout-at-bench-load)). Repeat on the carrier. The coupling that does bite is charging: the beacon out-draws the charger, so the battery is charged off the carrier.
